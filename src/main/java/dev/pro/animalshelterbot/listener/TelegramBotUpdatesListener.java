...
public class TelegramBotUpdatesListener implements UpdatesListener {
    ...
    private List<User> userDb; // = UserRepository.findAll()?
        ...
        for (Update update : updates) {
            ...
            if (!userDb.contains(chatId)) {
                ...
                if (incomingMessage.equalsIgnoreCase(String.valueOf(Commands.START))) {
                    chatConfigService.edit(DEFAULT);
...
//        finally{
    ...
    private SendResponse statusDeterminant(ChatConfig chatId) {
        BotStatus botStatus = BotStatus.valueOf(chatId.getChatState());
        ...
        switch (botStatus) {
            ...
            case CONSULT_NEW_USER:
                message = new SendMessage(chatId, Constants.CONSULT_NEW_USER);
                response = telegramBot.execute(message);
                break;
            case CONSULT_POTENTIAL_OWNER:
                message = new SendMessage(chatId, Constants.CONSULT_POTENTIAL_OWNER);
                response = telegramBot.execute(message);
                break;
            case KEEPING_a_PET:
...
//    public void createNotification(Update) throws DataFormatException {
